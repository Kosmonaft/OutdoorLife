...
class WonderPlugin_Carousel_Model {
	...
	function generate_body_code($id, $has_wrapper) {
		...
		if ($item_row != null)
		{
			...
			$boolOptions = array('autoplay', 'random', 'autoplayvideo', 'circular', 'responsive', 'showhoveroverlay', 'showhoveroverlayalways', 'lightboxresponsive', 'lightboxshownavigation', 'lightboxnogroup', 'lightboxshowtitle', 'lightboxshowdescription', 'usescreenquery');
	...
	function check_license($options) {
		...
		$key = trim( $options['wonderplugin-carousel-key'] );
		if ( empty($key) )
			return $ret;
